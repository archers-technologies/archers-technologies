...
const Hero = () => {
  ...
  const globeX = useTransform(scrollYProgress, [0, 1], isMobile ? ["2%", "-10%"] : ["7%", "-3%"]);
  const globeY = useTransform(scrollYProgress, [0, 1], isMobile ? ["4%", "-2%"] : ["7%", "3%"]);
  const globeScale = useTransform(scrollYProgress, [0, 1], isMobile ? [1.08, 0.98] : [1.12, 1.0]);
    ...
    <section ref={containerRef} className="relative h-[170vh]">
      <div className="sticky top-0 h-screen w-full overflow-hidden bg-black">
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-r from-black via-black/80 to-transparent w-[52%] sm:w-[48%] z-[1]" />
        ...
        <motion.div
          ...
          style={{
            x: reducedMotion ? (isMobile ? "2%" : "7%") : globeX,
            y: reducedMotion ? (isMobile ? "4%" : "7%") : globeY,
            scale: reducedMotion ? (isMobile ? 1.08 : 1.12) : globeScale,
        ...
        <motion.div
          className="relative z-10 h-full flex flex-col justify-center px-6 md:px-12 lg:px-16 xl:px-20 pt-16 pointer-events-none"
          style={{
        ...
        >
          <div className="max-w-md lg:max-w-lg pointer-events-auto">
            <h1 className="font-sans font-bold uppercase leading-[0.95] tracking-tight mb-8 md:mb-10">
              <span className="block text-3xl sm:text-4xl md:text-[2.75rem] lg:text-5xl text-white">
                We Build
              </span>
              <span className="block text-[2.125rem] sm:text-5xl md:text-[3.25rem] lg:text-[3.5rem] text-brand mt-1">
                Digital Products
              </span>
              <span className="block text-3xl sm:text-4xl md:text-[2.75rem] lg:text-5xl text-white mt-1">
            ...
            <div className="flex flex-col gap-4 md:gap-5">
              ...
              >
                <span className="flex items-center justify-center w-8 h-8 rounded-full border border-brand/50 group-hover:border-brand group-hover:bg-brand/10 transition-all">
                  <Play className="w-3 h-3 text-brand fill-brand ml-0.5" />
                </span>
                <span className="tracking-[0.15em] uppercase text-[11px] md:text-xs font-medium">
                  View Our Work
                </span>
              </Link>
        ...
        <button
          ...
          className="absolute bottom-7 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-2 text-muted-foreground/50 hover:text-brand/80 transition-colors"
